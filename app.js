...
function renderTable(key) {
  tableBody.innerHTML = "";
  const data = getData();
  // Si no hay datos previos, iniciar con 5 slots vacíos; si los hay, usar su largo (mínimo 1)
  const rows = data[key] && data[key].length ? data[key] : Array(5).fill({});
...
function addSlot() {
  if (!currentKey) return;
  const data = getData();
  const rows = data[currentKey] && data[currentKey].length ? [...data[currentKey]] : Array(5).fill({}).map(() => ({}));
  rows.push({});
